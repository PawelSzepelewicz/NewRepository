...
class AccountControllerTest(
    ...
    fun registerUser() {
        val createdUser = CreateUserDto("Person", "Info", "@Person0000", "person@gmail.com")
        ...
        mockMvc.perform(
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.description").value("Info"))
        ...
        repository.findByUsername("Person").let { person ->
    ...
    fun confirmRegistration() {
        ...
        repository.findByUsername("User")?.let {
            assertNotNull(it)
    ...
    fun blockAndUnblockUser() {
        ...
        repository.getUserById(1)?.apply { assertFalse(enabled) }
        mockMvc.perform(MockMvcRequestBuilders.post("/accounts/unblock/{userId}", 1)
                .header("Content-Type", "application/json"))
    ...
    @Throws(Exception::class)
    fun deleteUser() {
        mockMvc.perform(MockMvcRequestBuilders.delete("/accounts/{userId}", 1)
                .header("Content-Type", "application/json"))
        assertThrows<EntityNotFoundException> { service.getUserById(1) }
